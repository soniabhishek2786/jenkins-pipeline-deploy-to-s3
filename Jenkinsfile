...
pipeline {
    ...
    environment {
        ...
        AWS_DEFAULT_REGION = "us-east-1" // AWS region to use
    }
    ...
    stages {
        ...
        stage('Deploy to S3') {
            steps {
                ...
                script {
                    sh "aws s3 cp public/index.html s3://jenkinstf-ec2-static-bucket-Abhishek/"
                    sh "aws s3 cp public/error.html s3://jenkinstf-ec2-static-bucket-Abhishek/"
                }
            }
        }
        ...
        stage('Set Bucket Policy') {
            steps {
                ...
                script {
                    sh "aws s3api put-bucket-policy --bucket jenkinstf-ec2-static-bucket-Abhishek --policy file://policy.json"
                }
            }
        }
    }
